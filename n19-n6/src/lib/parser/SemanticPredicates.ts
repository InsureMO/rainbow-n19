import {Groovy, Objectweb} from '@rainbow-n19/n2';
...
export class SemanticPredicates {
	...
	static isFollowingArgumentsOrClosure(context: ExpressionContext): boolean {
		if (context instanceof PostfixExprAltContext) {
			...
			try {
				...
				throw new Groovy.GroovyBugError({
					message: 'Unexpected structure of expression context: ' + context,
					cause: e
				});
			}
		}
	...
	private static readonly MODIFIER_OPCODE_MAP: Map<number, number> = (() => {
		...
		map.set(GroovyParser.NATIVE, Objectweb.Opcodes.ACC_NATIVE);
		map.set(GroovyParser.SYNCHRONIZED, Objectweb.Opcodes.ACC_SYNCHRONIZED);
		map.set(GroovyParser.TRANSIENT, Objectweb.Opcodes.ACC_TRANSIENT);
		map.set(GroovyParser.VOLATILE, Objectweb.Opcodes.ACC_VOLATILE);

		map.set(GroovyParser.PUBLIC, Objectweb.Opcodes.ACC_PUBLIC);
		map.set(GroovyParser.PROTECTED, Objectweb.Opcodes.ACC_PROTECTED);
		map.set(GroovyParser.PRIVATE, Objectweb.Opcodes.ACC_PRIVATE);
		map.set(GroovyParser.STATIC, Objectweb.Opcodes.ACC_STATIC);
		map.set(GroovyParser.ABSTRACT, Objectweb.Opcodes.ACC_ABSTRACT);
		map.set(GroovyParser.SEALED, 0);
		map.set(GroovyParser.NON_SEALED, 0);
		map.set(GroovyParser.FINAL, Objectweb.Opcodes.ACC_FINAL);
		map.set(GroovyParser.STRICTFP, Objectweb.Opcodes.ACC_STRICT);
