...
describe('String literal rehydration tests: Single Quote and Triple Quotes', () => {
	beforeAll(() => {
		GroovyAstBuilder.enableTimeSpentLog();
	});

	test('SL, When with single quote started, Triple Quotes -> Single Quote + String Literal', async () => {
		const text = `'abc'''`;
	...
	test('SL, When with single quote started, Triple Quotes + Single Quote -> Single Quote + String Literal with triple quotes started', async () => {
		const text = `'abc''''`;
		const ast = GroovyAstBuilder.ast(text);
		AstChecker.check(ast, [
	...
	test('SL, When with triple quotes started, Single Quote -> Chars', async () => {
		const text = `'''abc'`;
		const ast = GroovyAstBuilder.ast(text);
		AstChecker.check(ast, [
			TokenId.COMPILATION_UNIT, 0, 7, 0, text, [
	...
	test('GL/SGL/DSGL, Single/Triple Quotes -> Chars', async () => {
		const text = `"''''"\n/''''/$/''''/$`;
		const ast = GroovyAstBuilder.ast(text);
		AstChecker.check(ast, [
